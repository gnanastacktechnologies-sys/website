...
const Hero = ({ settings }) => {
  ...
  // Helper to determine destination based on CTA text
  const getDestination = (text, defaultId) => {
    const t = text.toLowerCase();
    if (t.includes('product') || t.includes('explore') || t.includes('work')) return 'products';
    if (t.includes('project') || t.includes('contact') || t.includes('hire') || t.includes('start')) return 'contact';
    return defaultId;
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 grid md:grid-cols-2 gap-12 items-center">
        ...
        >
          ...
          <h1 className="text-4xl md:text-6xl font-extrabold text-white leading-tight mb-6">
            {settings.heroTitle.split('MERN Stack')[0]}
            <span className="gradient-text">MERN Stack</span>
            {settings.heroTitle.split('MERN Stack')[1]}
          </h1>
          ...
          <div className="flex flex-wrap gap-4">
            <button 
              onClick={() => scrollToSection(getDestination(settings.primaryCTA, 'contact'))}
              className="btn-primary"
            >
              {settings.primaryCTA}
            </button>
            <button 
              onClick={() => scrollToSection(getDestination(settings.secondaryCTA, 'products'))}
